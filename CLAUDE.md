...
nippon-color/
├── packages/
│   ├── core/        # @nippon-color/core — source of truth, deps: none
│   ├── css/         # @nippon-color/css  — deps: core
│   └── tailwind/    # @nippon-color/tailwind — deps: core
├── docs/            # VitePress site
...
`scripts/scrape.ts` (Playwright, one-time) → `packages/core/src/data/colors.json` (committed, static) → `@nippon-color/core` (JS exports) → consumed by `@nippon-color/css` and `@nippon-color/tailwind` at build time.

### Package Details

**`@nippon-color/core`** — built with unbuild (ESM + CJS dual output). Exports:
...
**`@nippon-color/css`** — `src/generate.ts` runs at build time to produce `dist/nippon-color.css`. Ships only the pre-built CSS file. Generates per-color `:root` custom properties (`--color-{romaji}`) and five utility classes: `.text-`, `.bg-`, `.border-`, `.fill-`, `.stroke-`.

**`@nippon-color/tailwind`** — built with unbuild (ESM + CJS). Plugin object with `handler: () => {}` and `config.theme.extend.colors.nippon` containing the full palette. Colors namespaced under `nippon-*` to avoid collision with Tailwind's built-in palette.
...
- `nippon-*` prefix in Tailwind prevents palette conflicts
